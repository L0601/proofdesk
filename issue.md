...
## 2026-04-24 PDF 跨页合并误吞下一页页眉

- 现象：`blk_000162` 在跨页合并后，把下一页页眉混入了正文。
- 项目：`ba26d5ef-1a5a-4873-8edd-eab773521322`
- 页码：第 41-42 页
- 对应文本：
  `要求省市三级医院选取 1-2 个病种建立联合诊治中心，县级医院重点加 38 丨 县域医共体建设与管理实践 强高转诊率疾病专科建设。`
...
- 不是数据库查询层串行错误。
- 问题发生在 PDF 导入分块阶段。
- `blk_000162` 的 `source_locator` 为 `page:41-42`，说明 block 在导入时已跨页合并。
- 下一页页眉 `38 丨 县域医共体建设与管理实践` 未被识别为运行页眉，误并入正文。

### 复现依据

- `document_blocks.text_content` 中已包含页眉文本。
- `json_payload.sourceMap.segments` 同时包含：
  - `page 41` 正文片段
  - `page 42, itemRange [0,4]` 页眉片段
  - `page 42, itemRange [5,23]` 正文片段
- 从截图可见，该页眉位于页面顶部，且与下方正文首段距离明显偏大。

### 命令结论

1. 当前问题本质上是 PDF 导入阶段的跨页误合并。
2. 仅靠文本规则识别页眉页脚不够稳，容易漏掉 `页码 + 分隔符 + 书名/章节名` 这类样式。
3. 页位置和与正文的垂直间距，是比纯文本更强的判定信号。

### 修复方案

- 在 `src/utils/pdfImport.ts` 中为段落补充位置信息：
  - `topY`
  - `bottomY`
  - `avgHeight`
  - `lineCount`
- 在跨页合并前，先按“位置 + 间距 + 文本模式”过滤运行页眉页脚。
- 文本规则保留兜底，至少覆盖：
  - 纯页码
  - `第 N 页`
  - `38 丨 县域医共体建设与管理实践` 这类 `页码 + 分隔符 + 标题` 格式

### 风险说明

- 仅靠文本模式补规则，后续仍可能漏掉更多变体页眉。
- 仅靠位置也可能误伤个别顶部短标题，因此需要位置与文本联合判定。
